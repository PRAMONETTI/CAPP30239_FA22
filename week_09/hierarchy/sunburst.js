...
.then((data) => {
  ...
  let rootNode = d3.hierarchy(data)
    .sum(d => d.value)
    .sort((a, b) => b.value - a.value); // Sort the values from the biggest to the smallest of the children
  ...
  svg.selectAll('path')
    ...
    .join('path')
    .attr('d', arc) // When the d is in quotes, it means that it goes to the HTML
    .attr('fill', d => {
